  ...
  if (!session) {
    return (
      <div style={{ padding: 16, display: "grid", gap: 10, background: "#0f172a", color: "#fff", borderRadius: 14 }}>
        <div style={{ fontWeight: 600 }}>🔒 Connexion par email</div>
        <input
          value={email}
        ...
        />
        <button onClick={() => email && signInWithMagicLink(email)} style={{ padding: "10px 12px", borderRadius: 10, background: "#fff", color: "#0f172a" }}>
          Recevoir le lien
        </button>
      </div>
    );
